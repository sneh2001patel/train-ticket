...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=r"^(Mysql/|/actuator|/swagger|/v2/api-docs|/webjars)",
...
def score_service(summary):
    count_weight = math.log10(summary["count"] + 1)
    return round(summary["cov"] * count_weight, 6)
...
def main():
    ...
    for service, latencies in service_latencies.items():
        ...
        for (endpoint_service, endpoint_name), endpoint_values in endpoint_latencies.items():
            ...
            if top_endpoint_summary is None or candidate["cov"] > top_endpoint_summary["cov"]:
                top_endpoint = endpoint_name
                top_endpoint_summary = candidate

        entry = {
            "service": service,
            "score": score_service(summary),
            **summary,
